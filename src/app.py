...
with st.container():
    ...
    with residence_type_sec:
        residence_type_value = st.selectbox(
            label="Residence Type", options=residence_type_lbl
        )
    ...
    with bmi_sec:
        bmi_value = st.number_input(
            label="BMI", min_value=10.0, max_value=100.0, value=28.0
        )
        bmi_value = [bmi_value]

    with smoking_status_sec:
        smoking_status_value = st.selectbox(
            "Smoking Status", options=smoking_status_lbl
        )
...
features = (
    ...
    + hypertension_value
    + heart_disease_value
    + avg_glucose_level_value
    + bmi_value
    + gender_value
    + ever_married_value
    + work_type_value
    + residence_type_value
...
with st.container():
    st_shap(
        shap.force_plot(explainer.expected_value[1], shap_values[1], df_features_input)
    )
